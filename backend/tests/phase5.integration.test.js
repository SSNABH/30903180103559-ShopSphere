...
test('Phase 5 services deliver email, logs, and Admin statistics', async (t) => {
  ...
  // Reviews live in the review service now. Statistics reaches it over REST,
  // so the stub stands in for that client.
  const reviewSource = { async countAll() { return 1; } };
  ...
  const app = createApp({ ...memory, reviewSource, activityLogService, emailService, statisticsRepository });
  ...
  // The monolith no longer serves review routes at all, which is the point of
  // the extraction. Asking for one proves the logic really left.
  const goneFromMonolith = await request(baseUrl, `/products/${productId}/reviews`);
  assert.equal(goneFromMonolith.response.status, 404);
