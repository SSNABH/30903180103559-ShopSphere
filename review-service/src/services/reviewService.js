...
import { fetchProduct } from '../clients/shopsphereApi.js';
...
export function createReviewService(reviewRepository, activityLogService, productLookup = fetchProduct) {
  // In the monolith this read the product straight out of Postgres through
  // productRepository. The catalogue stayed behind in the main application, so
  // the same check is now a REST call.
  async function productFor(identifier) {
    const product = await productLookup(identifier);
  ...
  return {
    ...
    async create(identifier, user, input) {
      ...
      await activityLogService.record({
        ...
        entityType: 'REVIEW',
        entityId: String(review._id ?? review.id),
        metadata: { productId: product.id, rating: input.rating },
      });
    ...
    // Consumed by the main application's admin statistics, which used to read
    // this straight from the reviews collection.
    countAll() {
